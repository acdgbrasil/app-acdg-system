...
## Current Phase
phase: in-progress (C07 + C08 closed; **C09 next** — cli-team)
agent: —
...
- [x] **C08 — cli-lookup** — CLOSED 2026-05-04 via pipeline 4-wave (W0 ~110 RED → W1 565 GREEN → W2 APPROVED Round 1 com 0 MUST_FIX → W3 PASSED). **Maior ticket da Phase 5: 9 endpoints + sub-parent + 5º HTTP verb (PATCH).** Shape NESTED: `lookup get|batch|create|update|toggle|request` + `lookup request list|create|approve|reject` (singular `request` consistente). 565 GREEN no apps/cli (era 450, +115); 2237 GREEN reachable workspace (Δ +115). **`BffClient.patch<T>` adicionado** mirror de `put<T>` reutilizando `_attempt`/`_refreshAndRetry` (`method` String já genérico). **`BffClient.get<T>` extendido com `queryParameters` map** — backward-compat (default null; C02-C07 callers unchanged). DTO-as-canon 7ª aplicação: `--code → codigo`, `--label → descricao` (translation pin). `lookup batch` cap-20 client-side (mirrors `GetLookupsBatchIntent.maxTables = 20`). `lookup toggle --active` REQUIRED + JSON bool wire type. `lookup update` body exclui `active` (DTO rejects). `decodeStandardIdResponse` agora com 5 call-sites (care/appointment + protection/violation+referral + lookup/create+request-create).
...
Next action: kickoff **C09 — cli-team** (9 endpoints, último ticket da Onda 4). Pré-req: `BffClient` cobre todos os 5 verbs (GET/POST/PUT/DELETE/PATCH); `_command_helpers.dart` (`decodeStandardIdResponse` com 5 call-sites) + `_yaml_helpers.dart` disponíveis. Team sub-contract em `apps/social_care_bff/contracts/lib/src/contract/sub_contracts/team_contract.dart`. Pipeline 4-wave igual. **Após C09 fechar Onda 4**, segue Onda 5: C10 (golden tests) + C11 (docs).
