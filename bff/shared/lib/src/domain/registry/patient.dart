...
final class Patient with Equatable {
  ...
  static Result<Patient> create({
    ...
    if (diagnoses.isEmpty) {
      return Failure(_buildError('PAT-003', 'Ao menos um diagnóstico (CID) é obrigatório para abertura de prontuário.'));
    }
    ...
    final prCount = familyMembers.where((m) => m.relationshipId == prRelationshipId).length;
    if (prCount == 0) {
      return Failure(_buildError('PAT-008', 'É necessário exatamente uma Pessoa de Referência (PR) na composição familiar.'));
    }
    if (prCount > 1) {
      return Failure(_buildError('PAT-009', 'Não é permitido mais de uma Pessoa de Referência (PR).'));
    }
  ...
  Patient copyWith({
    ...
    return Patient._(
      ...
      housingCondition: housingCondition != null ? housingCondition() : this.housingCondition,
      socioeconomicSituation: socioeconomicSituation != null ? socioeconomicSituation() : this.socioeconomicSituation,
      workAndIncome: workAndIncome != null ? workAndIncome() : this.workAndIncome,
      educationalStatus: educationalStatus != null ? educationalStatus() : this.educationalStatus,
      healthStatus: healthStatus != null ? healthStatus() : this.healthStatus,
      communitySupportNetwork: communitySupportNetwork != null ? communitySupportNetwork() : this.communitySupportNetwork,
      socialHealthSummary: socialHealthSummary != null ? socialHealthSummary() : this.socialHealthSummary,
      appointments: appointments ?? this.appointments,
      referrals: referrals ?? this.referrals,
      violationReports: violationReports ?? this.violationReports,
      placementHistory: placementHistory != null ? placementHistory() : this.placementHistory,
