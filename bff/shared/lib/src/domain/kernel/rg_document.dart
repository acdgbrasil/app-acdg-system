...
final class RgDocument with Equatable {
  ...
  String get formattedNumber =>
      '${number.substring(0, 8)}-${number.substring(8)}';
  ...
  static Result<RgDocument> create({
    ...
    if (!RegExp(r'^[0-9]{8}[0-9X]$').hasMatch(rawNumber)) {
      return Failure(
        _buildError(
          'RGD-005',
          'RG deve ter 8 dígitos numéricos seguidos de 1 dígito verificador.',
        ),
      );
    }
    ...
    if (!_isValidCheckDigit(rawNumber)) {
      return Failure(
        _buildError('RGD-006', 'Dígito verificador do RG é inválido.'),
      );
    }
    ...
    if (issueDate.date.isAfter(referenceNow.date)) {
      return Failure(
        _buildError('RGD-004', 'Data de emissão do RG não pode ser no futuro.'),
      );
    }
  ...
  static bool _isValidCheckDigit(String rg) {
    ...
    if (remainder == 10) {
      ...
      expectedDigit =
          '0'; // Based on specification, sum % 11 will never be 11. But treating 11 - rem logic.
