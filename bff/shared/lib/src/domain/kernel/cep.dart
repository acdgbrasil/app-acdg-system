...
///
/// Extension type zero-cost sobre `String` (8 dígitos numéricos).
extension type const Cep._(String value) {
  /// Origem garantida — zero-cost. Use com parcimônia.
  const Cep.trusted(String value) : this._(value);
  ...
  static Result<Cep> create(String? rawValue) {
    if (rawValue == null || rawValue.normalizedTrim().isEmpty) {
      return Failure(_buildCepError('CEP-001', 'O CEP não pode ser vazio.'));
    }

    final trimmed = rawValue.normalizedTrim();
    if (!RegExp(r'^[\d\-\s]+$').hasMatch(trimmed)) {
      return Failure(
        _buildCepError('CEP-002', 'O CEP contém caracteres inválidos.'),
      );
    }
    ...
    if (digits.length != 8) {
      return Failure(
        _buildCepError('CEP-003', 'O CEP deve conter exatamente 8 dígitos.'),
      );
    }
...
bool _isValidStateRange(int cep) {
  final ranges = [
    [1000000, 19999999], // SP
...
AppError _buildCepError(String code, String message) {
  return AppError(
    code: code,
    message: message,
