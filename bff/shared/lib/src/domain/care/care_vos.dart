  ...
  static Result<IcdCode> create(
    ...
    if (requiresDot && !val.contains('.')) {
      return Failure(
        AppError(
          code: 'ICD-002',
          message: 'Formato inválido de CID',
...
final class Diagnosis with Equatable {
  ...
  Diagnosis copyWith({IcdCode? id, TimeStamp? date, String? description}) {
    return Diagnosis._(
      id: id ?? this.id,
      date: date ?? this.date,
  ...
  static Result<Diagnosis> create({
    required IcdCode id,
    ...
    if (date.date.isAfter(refNow.date)) {
      return Failure(
        _buildDiagError(
          'DIA-001',
          'Data do diagnóstico não pode ser no futuro',
        ),
      );
    }
    ...
    if (desc == null || desc.isEmpty) {
      return Failure(
        _buildDiagError(
          'DIA-003',
          'Descrição do diagnóstico não pode ser vazia',
        ),
      );
    }
...
final class IngressInfo with Equatable {
  ...
  IngressInfo copyWith({
    ...
    return IngressInfo._(
      ...
      originContact: originContact != null
          ? originContact()
          : this.originContact,
  ...
  static Result<IngressInfo> create({
    required LookupId ingressTypeId,
    String? originName,
    String? originContact,
    required String? serviceReason,
    required List<ProgramLink> linkedSocialPrograms,
  }) {
    ...
    if (reason == null || reason.isEmpty) {
      return Failure(
        AppError(
          ...
          module: 'social-care/ingress-info',
          kind: 'domainValidation',
          http: 422,
          observability: const Observability(
            category: ErrorCategory.domainRuleViolation,
...
final class SocialCareAppointment with Equatable {
  ...
  SocialCareAppointment copyWith({
    ...
    return SocialCareAppointment._(
      ...
      professionalInChargeId:
          professionalInChargeId ?? this.professionalInChargeId,
      type: type ?? this.type,
  ...
  static Result<SocialCareAppointment> create({
    ...
    required TimeStamp date,
    required ProfessionalId professionalInChargeId,
    ...
    if (s == null && ap == null) {
      return Failure(
        _buildApptError(
          'SCA-003',
          "Pelo menos um dos campos 'resumo' ou 'plano de ação' deve ser preenchido",
        ),
