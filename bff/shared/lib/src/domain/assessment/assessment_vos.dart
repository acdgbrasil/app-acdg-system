  ...
  static Result<HousingCondition> create({
    ...
    if (numberOfBedrooms > numberOfRooms) {
      return Failure(
        _buildError(
          'HC-004',
          'Número de quartos não pode exceder o número total de cômodos',
        ),
      );
    }
...
final class SocioEconomicSituation with Equatable {
  ...
  static Result<SocioEconomicSituation> create({
    ...
    required double incomePerCapita,
    required bool receivesSocialBenefit,
    required SocialBenefitsCollection socialBenefits,
    ...
    if (!receivesSocialBenefit && !socialBenefits.isEmpty) {
      return Failure(
        _buildSocioError(
          'SES-001',
          'Inconsistência: marcou que não recebe benefício, mas há benefícios cadastrados',
        ),
      );
    }
    if (receivesSocialBenefit && socialBenefits.isEmpty) {
      return Failure(
        _buildSocioError(
          'SES-002',
          'Inconsistência: marcou que recebe benefício, mas nenhum benefício foi informado',
        ),
      );
    }

    return Success(
      SocioEconomicSituation._(
  ...
  static AppError _buildSocioError(String code, String message) {
    return AppError(
      code: code,
      message: message,
      module: 'social-care/socio-economic',
      kind: 'domainValidation',
      http: 422,
      observability: const Observability(
        category: ErrorCategory.domainRuleViolation,
