...
final class CommunitySupportNetwork with Equatable {
  ...
  @override
  List<Object?> get props => [hasRelativeSupport, hasNeighborSupport, familyConflicts, patientParticipatesInGroups, familyParticipatesInGroups, patientHasAccessToLeisure, facesDiscrimination];
  ...
  static Result<CommunitySupportNetwork> create({
    ...
    if (familyConflicts != null && conflicts.isEmpty && familyConflicts.trim().isEmpty && familyConflicts.isNotEmpty) {
       return Failure(_buildError('CSN-001', 'Conflitos familiares não pode conter apenas espaços em branco'));
    }

    if (conflicts.length > 300) {
      return Failure(_buildError('CSN-002', 'Conflitos familiares não pode exceder 300 caracteres'));
    }

    return Success(CommunitySupportNetwork._(
  ...
  static AppError _buildError(String code, String message) {
    return AppError(code: code, message: message, module: 'social-care/community-support', kind: 'domainValidation', http: 422, observability: const Observability(category: ErrorCategory.domainRuleViolation, severity: ErrorSeverity.warning));
  }
}
