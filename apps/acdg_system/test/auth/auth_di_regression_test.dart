...
void main() {
  group('Auth DI Regression & Blindagem', () {
    ...
    test(
      'REGRESSÃO: AuthViewModel e AppRouter DEVEM compartilhar a mesma instância via Riverpod',
      () {
        ...
        expect(
          ...
          same(authVM),
          reason:
              'ALERTA: Instâncias duplicadas detectadas! O AppRouter deve usar o VM do Riverpod.',
        );
      },
    );

    test(
      'REGRESSÃO: AuthViewModel DEVE sincronizar estado inicial do repositório (Eliminação do Ponto Cego)',
      () async {
        ...
        final preAuthContainer = ProviderContainer(
          overrides: [authRepositoryProvider.overrideWithValue(preAuthRepo)],
        );
        ...
        expect(
          ...
          isA<Authenticated>(),
          reason:
              'ALERTA: O ViewModel falhou em ler o estado inicial do repositório. Risco de ficar preso no Splash!',
        );
      },
    );

    test(
      'BLINDAGEM: AppRouter DEVE reagir a mudanças de estado subsequentes',
      () async {
        final authVM = container.read(authViewModelProvider);
        final router = container.read(appRouterProvider);
        ...
        expect(
          router.authViewModel.status,
          isA<Authenticated>(),
          reason:
              'O Router não reagiu à mudança de status do ViewModel principal.',
        );
      },
    );

    test(
      'BLINDAGEM: authInitializationProvider DEVE garantir a execução do init()',
      () async {
        ...
        expect(
          ...
          isA<Authenticated>(),
          reason:
              'O authInitializationProvider não chamou o restoreSessionUseCase corretamente.',
        );
      },
    );
  });
