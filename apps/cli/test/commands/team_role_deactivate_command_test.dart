/// W0 RED — `TeamRoleDeactivateCommand` orchestration contract (C09).
///
/// W1 must create
/// `apps/cli/lib/src/commands/team_role_deactivate_command.dart`:
///
/// ```dart
/// class TeamRoleDeactivateCommand extends Command<int> {
...
/// ```
///
/// Behavior (per `DeactivateRoleIntent.parseFromParams` (path-only, two
/// positionals) + `TeamContract.deactivateRole`):
...
///   * PUT `/team/<member-id>/roles/<role-id>/deactivate` → 200/204 → exit 0.
///   * Missing either positional → usage error 64, no BFF call.
///   * 400 INVALID_DEACTIVATE_ROLE_PARAMS / 404 / 401 / 5xx → non-zero
...
import 'package:cli/src/commands/team_role_deactivate_command.dart';
...
void main() {
  group('TeamRoleDeactivateCommand basics', () {
    test('extends Command<int> with name "deactivate"', () {
      final cmd = TeamRoleDeactivateCommand(
  ...
  group('TeamRoleDeactivateCommand — happy path', () {
    test(
      'PUTs /team/<member-id>/roles/<role-id>/deactivate with NO body',
      () async {
        ...
        expect(
          ...
          equals('/team/$_kMemberId/roles/$_kRoleId/deactivate'),
        );
  ...
  group('TeamRoleDeactivateCommand — failure paths', () {
    ...
    test('BFF 400 INVALID_DEACTIVATE_ROLE_PARAMS (UUID failure) → '
        'non-zero exit + stderr', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"INVALID_DEACTIVATE_ROLE_PARAMS",'
