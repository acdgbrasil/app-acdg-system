/// W0 RED — `TeamRoleReactivateCommand` orchestration contract (C09).
///
/// W1 must create
/// `apps/cli/lib/src/commands/team_role_reactivate_command.dart`:
///
/// ```dart
/// class TeamRoleReactivateCommand extends Command<int> {
///   TeamRoleReactivateCommand({
///     required this.bffClient,
///     required this.formatter,
///     this.stdout,
///     this.stderr,
///   });
///
///   final BffClient bffClient;
///   final OutputFormatter formatter;
///   final StringSink? stdout;
///   final StringSink? stderr;
///
///   @override String get name => 'reactivate';
///   @override Future<int> run();
/// }
/// ```
///
/// Behavior (per `ReactivateRoleIntent.parseFromParams` (path-only, two
/// positionals) + `TeamContract.reactivateRole`):
///   * Required positional `<member-id>` — first positional argument.
///   * Required positional `<role-id>`   — second positional argument.
///     Both validated as UUID v4 by the BFF intent — UUID failure
///     short-circuits left-to-right (memberId first, then roleId) per
///     `reactivate_role_intent.dart:21-23`.
///   * **No body** — path-only PUT.
///   * PUT `/team/<member-id>/roles/<role-id>/reactivate` → 200/204 → exit 0.
///   * Missing either positional → usage error 64, no BFF call.
///   * 400 INVALID_REACTIVATE_ROLE_PARAMS / 404 / 401 / 5xx → non-zero
///     exit + status code in stderr.
library;

import 'dart:typed_data';

import 'package:args/command_runner.dart';
import 'package:dio/dio.dart';
import 'package:test/test.dart';

import 'package:cli/src/commands/team_role_reactivate_command.dart';
import 'package:cli/src/formatters/json_formatter.dart';
import 'package:cli/src/session/bff_client.dart';
import 'package:cli/src/session/credential_store.dart';
import 'package:cli/src/session/oidc_session.dart';

const String _kMemberId = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';
const String _kRoleId = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
const String _kVoidResponseJson =
    '{"data":null,"meta":{"timestamp":"2026-05-04T12:00:00Z"}}';

void main() {
  group('TeamRoleReactivateCommand basics', () {
    test('extends Command<int> with name "reactivate"', () {
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(_CapturingAdapter(_kVoidResponseJson, status: 200)),
        formatter: const JsonFormatter(),
      );
      expect(cmd, isA<Command<int>>());
      expect(cmd.name, equals('reactivate'));
      expect(cmd.description, isNotEmpty);
    });
  });

  group('TeamRoleReactivateCommand — happy path', () {
    test(
      'PUTs /team/<member-id>/roles/<role-id>/reactivate with NO body',
      () async {
        final adapter = _CapturingAdapter(_kVoidResponseJson, status: 200);
        final cmd = TeamRoleReactivateCommand(
          bffClient: _bff(adapter),
          formatter: const JsonFormatter(),
          stdout: StringBuffer(),
        );

        final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

        expect(exit, equals(0));
        expect(adapter.lastOptions!.method.toUpperCase(), equals('PUT'));
        expect(
          adapter.lastOptions!.path,
          equals('/team/$_kMemberId/roles/$_kRoleId/reactivate'),
        );
        // No body on a path-only PUT.
        final raw = adapter.lastOptions!.data;
        expect(
          raw,
          anyOf(isNull, equals(''), equals(<String, Object?>{}), isEmpty),
        );
      },
    );
  });

  group('TeamRoleReactivateCommand — failure paths', () {
    test('missing both positionals → non-zero exit, no BFF call', () async {
      final adapter = _CapturingAdapter(_kVoidResponseJson, status: 200);
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const []);

      expect(exit, isNot(equals(0)));
      expect(adapter.lastOptions, isNull);
    });

    test(
      'missing second positional (role-id) → non-zero exit, no BFF call',
      () async {
        final adapter = _CapturingAdapter(_kVoidResponseJson, status: 200);
        final cmd = TeamRoleReactivateCommand(
          bffClient: _bff(adapter),
          formatter: const JsonFormatter(),
          stderr: StringBuffer(),
        );

        final exit = await _runWithArgs(cmd, const [_kMemberId]);

        expect(exit, isNot(equals(0)));
        expect(adapter.lastOptions, isNull);
      },
    );

    test('BFF 400 INVALID_REACTIVATE_ROLE_PARAMS (UUID failure) → '
        'non-zero exit + stderr', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"INVALID_REACTIVATE_ROLE_PARAMS",'
        '"message":"roleId must be UUID v4"}}',
        status: 400,
      );
      final stderr = StringBuffer();
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString(), contains('400'));
    });

    test('BFF 404 → non-zero exit + stderr surfaces status', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"ROLE_NOT_FOUND","message":"role not found"}}',
        status: 404,
      );
      final stderr = StringBuffer();
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString(), contains('404'));
    });

    test('BFF 401 → non-zero exit + auth-related stderr', () async {
      final adapter = _CapturingAdapter('unauthorized', status: 401);
      final stderr = StringBuffer();
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString().toLowerCase(), contains('auth'));
    });

    test('BFF 500 → non-zero exit + stderr surfaces status', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"INTERNAL","message":"Internal server error"}}',
        status: 500,
      );
      final stderr = StringBuffer();
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString(), contains('500'));
    });

    test('Network failure → non-zero exit', () async {
      final cmd = TeamRoleReactivateCommand(
        bffClient: _bff(_ThrowingAdapter()),
        formatter: const JsonFormatter(),
        stderr: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const [_kMemberId, _kRoleId]);

      expect(exit, isNot(equals(0)));
    });
  });
}

// ---------------------------------------------------------------------------
// Fixtures + fakes
// ---------------------------------------------------------------------------

BffClient _bff(HttpClientAdapter adapter) {
  final dio = Dio(BaseOptions(baseUrl: 'http://localhost:3000'))
    ..httpClientAdapter = adapter;
  return BffClient(
    baseUrl: 'http://localhost:3000',
    credentialStore: _NullStore(),
    dio: dio,
  );
}

class _NullStore implements CredentialStore {
  @override
  Future<OidcSession?> read() async => null;
  @override
  Future<void> write(OidcSession session) async {}
  @override
  Future<void> clear() async {}
}

class _CapturingAdapter implements HttpClientAdapter {
  _CapturingAdapter(this._body, {required this.status});

  final String _body;
  final int status;
  RequestOptions? lastOptions;

  @override
  void close({bool force = false}) {}

  @override
  Future<ResponseBody> fetch(
    RequestOptions options,
    Stream<Uint8List>? requestStream,
    Future<void>? cancelFuture,
  ) async {
    lastOptions = options;
    return ResponseBody.fromString(
      _body,
      status,
      headers: {
        'content-type': ['application/json'],
      },
    );
  }
}

class _ThrowingAdapter implements HttpClientAdapter {
  @override
  void close({bool force = false}) {}

  @override
  Future<ResponseBody> fetch(
    RequestOptions options,
    Stream<Uint8List>? requestStream,
    Future<void>? cancelFuture,
  ) async {
    throw DioException(
      requestOptions: options,
      type: DioExceptionType.connectionError,
      message: 'simulated network failure',
    );
  }
}

Future<int> _runWithArgs(Command<int> cmd, List<String> args) async {
  final runner = CommandRunner<int>('test', 'test')..addCommand(cmd);
  try {
    final code = await runner.run([cmd.name, ...args]);
    return code ?? 0;
  } on UsageException {
    return 64;
  }
}
