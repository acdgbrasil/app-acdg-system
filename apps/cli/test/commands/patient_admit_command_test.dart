/// W0 RED — `PatientAdmitCommand` orchestration contract (C03).
///
/// W1 must create `apps/cli/lib/src/commands/patient_admit_command.dart`:
///
/// ```dart
/// class PatientAdmitCommand extends Command<int> {
///   PatientAdmitCommand({
///     required this.bffClient,
///     required this.formatter,
///     this.stdout,
///     this.stderr,
///   }) {
///     argParser
///       ..addOption('reason', help: 'Reason for admit (required)')
///       ..addOption('admitted-at',
///           help: 'ISO8601 admit timestamp (required)')
///       ..addOption('notes', help: 'Optional notes');
///   }
///   @override String get name => 'admit';
///   @override Future<int> run();
/// }
/// ```
///
/// Behavior (Contract `AdmitPatientRequest` shape):
///   * Required positional `<patient-id>`.
///   * Required `--reason` and `--admitted-at` (ISO8601 string).
///   * Optional `--notes`.
///   * Validate `--admitted-at` parses as ISO8601 (`DateTime.parse`); on
///     failure → usage error, no BFF call.
///   * POST `/patients/<id>/admit` body:
///       `{"reason": ..., "admittedAt": ..., "notes": ...}`
library;

import 'dart:convert';
import 'dart:typed_data';

import 'package:args/command_runner.dart';
import 'package:dio/dio.dart';
import 'package:test/test.dart';

import 'package:cli/src/commands/patient_admit_command.dart';
import 'package:cli/src/formatters/json_formatter.dart';
import 'package:cli/src/session/bff_client.dart';
import 'package:cli/src/session/credential_store.dart';
import 'package:cli/src/session/oidc_session.dart';

const String _kPatientId = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';

void main() {
  group('PatientAdmitCommand basics', () {
    test('extends Command<int> with name "admit"', () {
      final cmd = PatientAdmitCommand(
        bffClient: _bff(_CapturingAdapter('{}', status: 204)),
        formatter: const JsonFormatter(),
      );
      expect(cmd, isA<Command<int>>());
      expect(cmd.name, equals('admit'));
      expect(cmd.description, isNotEmpty);
    });

    test('argParser declares reason / admitted-at / notes options', () {
      final cmd = PatientAdmitCommand(
        bffClient: _bff(_CapturingAdapter('{}', status: 204)),
        formatter: const JsonFormatter(),
      );
      final options = cmd.argParser.options.keys.toSet();
      expect(options, contains('reason'));
      expect(options, contains('admitted-at'));
      expect(options, contains('notes'));
    });
  });

  group('PatientAdmitCommand — happy path', () {
    test('POSTs /patients/<id>/admit with reason + admittedAt body', () async {
      final adapter = _CapturingAdapter('', status: 204);
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stdout: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const [
        _kPatientId,
        '--reason=Waitlist promotion',
        '--admitted-at=2026-04-30T10:00:00Z',
        '--notes=verified by social worker',
      ]);

      expect(exit, equals(0));
      expect(adapter.lastOptions!.method.toUpperCase(), equals('POST'));
      expect(adapter.lastOptions!.path, equals('/patients/$_kPatientId/admit'));
      final body = _decodeBody(adapter.lastOptions!.data);
      expect(body['reason'], equals('Waitlist promotion'));
      expect(body['admittedAt'], equals('2026-04-30T10:00:00Z'));
      expect(body['notes'], equals('verified by social worker'));
    });
  });

  group('PatientAdmitCommand — failure paths', () {
    test('missing positional patient-id → non-zero exit', () async {
      final cmd = PatientAdmitCommand(
        bffClient: _bff(_CapturingAdapter('', status: 204)),
        formatter: const JsonFormatter(),
      );

      final exit = await _runWithArgs(cmd, const [
        '--reason=x',
        '--admitted-at=2026-04-30T10:00:00Z',
      ]);

      expect(exit, isNot(equals(0)));
    });

    test('missing --reason → non-zero exit, no BFF call', () async {
      final adapter = _CapturingAdapter('', status: 204);
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const [
        _kPatientId,
        '--admitted-at=2026-04-30T10:00:00Z',
      ]);

      expect(exit, isNot(equals(0)));
      expect(adapter.lastOptions, isNull);
    });

    test('missing --admitted-at → non-zero exit, no BFF call', () async {
      final adapter = _CapturingAdapter('', status: 204);
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const [_kPatientId, '--reason=x']);

      expect(exit, isNot(equals(0)));
      expect(adapter.lastOptions, isNull);
    });

    test('--admitted-at not ISO8601 → non-zero exit, no BFF call', () async {
      final adapter = _CapturingAdapter('', status: 204);
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: StringBuffer(),
      );

      final exit = await _runWithArgs(cmd, const [
        _kPatientId,
        '--reason=x',
        '--admitted-at=not-a-date',
      ]);

      expect(exit, isNot(equals(0)));
      expect(adapter.lastOptions, isNull);
    });

    test('BFF 409 → non-zero exit + stderr', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"CONFLICT","message":"Patient already admitted"}}',
        status: 409,
      );
      final stderr = StringBuffer();
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [
        _kPatientId,
        '--reason=x',
        '--admitted-at=2026-04-30T10:00:00Z',
      ]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString(), contains('409'));
    });

    test('BFF 401 → non-zero exit + auth-related stderr', () async {
      final adapter = _CapturingAdapter('unauthorized', status: 401);
      final stderr = StringBuffer();
      final cmd = PatientAdmitCommand(
        bffClient: _bff(adapter),
        formatter: const JsonFormatter(),
        stderr: stderr,
      );

      final exit = await _runWithArgs(cmd, const [
        _kPatientId,
        '--reason=x',
        '--admitted-at=2026-04-30T10:00:00Z',
      ]);

      expect(exit, isNot(equals(0)));
      expect(stderr.toString().toLowerCase(), contains('auth'));
    });
  });
}

// ---------------------------------------------------------------------------
// Fixtures + fakes
// ---------------------------------------------------------------------------

BffClient _bff(HttpClientAdapter adapter) {
  final dio = Dio(BaseOptions(baseUrl: 'http://localhost:3000'))
    ..httpClientAdapter = adapter;
  return BffClient(
    baseUrl: 'http://localhost:3000',
    credentialStore: _NullStore(),
    dio: dio,
  );
}

class _NullStore implements CredentialStore {
  @override
  Future<OidcSession?> read() async => null;
  @override
  Future<void> write(OidcSession session) async {}
  @override
  Future<void> clear() async {}
}

class _CapturingAdapter implements HttpClientAdapter {
  _CapturingAdapter(this._body, {required this.status});

  final String _body;
  final int status;
  RequestOptions? lastOptions;

  @override
  void close({bool force = false}) {}

  @override
  Future<ResponseBody> fetch(
    RequestOptions options,
    Stream<Uint8List>? requestStream,
    Future<void>? cancelFuture,
  ) async {
    lastOptions = options;
    return ResponseBody.fromString(
      _body,
      status,
      headers: {
        'content-type': ['application/json'],
      },
    );
  }
}

Map<String, Object?> _decodeBody(Object? raw) {
  if (raw is Map<String, Object?>) return raw;
  if (raw is String) {
    final decoded = jsonDecode(raw);
    return decoded as Map<String, Object?>;
  }
  fail('Unexpected request body shape: ${raw.runtimeType}');
}

Future<int> _runWithArgs(Command<int> cmd, List<String> args) async {
  final runner = CommandRunner<int>('test', 'test')..addCommand(cmd);
  try {
    final code = await runner.run([cmd.name, ...args]);
    return code ?? 0;
  } on UsageException {
    return 64;
  }
}
