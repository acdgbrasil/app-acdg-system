/// W0 RED — `TeamReactivateCommand` orchestration contract (C09).
///
/// W1 must create `apps/cli/lib/src/commands/team_reactivate_command.dart`:
///
/// ```dart
/// class TeamReactivateCommand extends Command<int> {
...
/// ```
///
/// Behavior (per `ReactivateWorkerIntent.parseFromPath` (path-only) +
/// `TeamContract.reactivateWorker`):
///   * Required positional `<member-id>` (UUID v4 — BFF intent gates).
///   * **No body** — path-only PUT.
///   * PUT `/team/<member-id>/reactivate` → 200/204 → exit 0.
///   * 400 INVALID_REACTIVATE_WORKER_PARAMS (UUID failure) / 404 / 401 /
...
import 'package:cli/src/commands/team_reactivate_command.dart';
...
void main() {
  group('TeamReactivateCommand basics', () {
    test('extends Command<int> with name "reactivate"', () {
      final cmd = TeamReactivateCommand(
  ...
  group('TeamReactivateCommand — happy path', () {
    test('PUTs /team/<id>/reactivate with NO body', () async {
      ...
      expect(adapter.lastOptions!.path, equals('/team/$_kMemberId/reactivate'));
  ...
  group('TeamReactivateCommand — failure paths', () {
    ...
    test('BFF 400 INVALID_REACTIVATE_WORKER_PARAMS (UUID failure) → '
        'non-zero exit + stderr', () async {
      final adapter = _CapturingAdapter(
        '{"error":{"code":"INVALID_REACTIVATE_WORKER_PARAMS",'
