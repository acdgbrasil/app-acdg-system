...
/// Contract for [ReactivateRoleIntent] post-A23 (Template B V2 — 2 path UUIDs).
///
/// Path-only intent for `PUT /team/{memberId}/roles/{roleId}/reactivate`.
...
void main() {
  group('ReactivateRoleIntent', () {
    test('constructs with required memberId and roleId', () {
      const intent = ReactivateRoleIntent(
    ...
    group('parseFromParams — Result<ReactivateRoleIntent>', () {
      test('returns Success when both ids are valid UUID v4', () {
        ...
        expect(result, isA<Success<ReactivateRoleIntent>>());
        switch (result) {
      ...
      test(
        ...
        () {
          ...
          expect(result, isA<Failure<ReactivateRoleIntent>>());
          switch (result) {
            case Success():
      ...
      test('rejects v1 UUID in either slot (only v4 allowed)', () {
        ...
        final r2 = ReactivateRoleIntent.parseFromParams(
          rawMemberId: kMemberUuid,
          rawRoleId: kUuidV1,
        );

        expect(r1, isA<Failure<ReactivateRoleIntent>>());
        expect(r2, isA<Failure<ReactivateRoleIntent>>());
      });
    });
  });
}
