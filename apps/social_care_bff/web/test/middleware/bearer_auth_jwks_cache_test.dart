...
void main() {
  group('G. JWKS cache', () {
    ...
    test('Test #47: 100 concurrent cold-start requests → 1 JWKS fetch '
        '(single-flight)', () async {
      // Slow the fetch to widen the contention window.
      jwksClient.injectDelay(const Duration(milliseconds: 50));

      final futures = List.generate(100, (_) => jwksCache.getKey(kValidKid));
    ...
    test(
      'Test #48: JWKS endpoint timeout (5s) → fail-closed, 401 within ~5.5s',
      () async {
    ...
    test(
      'Test #50: rotation — old kid stays valid until TTL expires',
      () async {
    ...
    test('Test #51: refresh on signature mismatch — known kid, sig fails → '
        '1 refetch, retry, then 401', () async {
