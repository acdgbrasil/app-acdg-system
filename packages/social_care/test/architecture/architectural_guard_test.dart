  ...
  group('ACDG Architectural Guard: Enforcement', () {
      ...
      for (final dir in targetDirs) {
        for (final file in allDartFiles(dir)) {
          ...
          if (content.contains('.valueOrNull!')) {
            violations.add('${file.path}: Found dangerous use of .valueOrNull!. Use pattern matching instead.');
          }
        }
      }
      if (violations.isNotEmpty) fail('Architectural Violations (Safety):\n${violations.join('\n')}');
    });

    test('RULE: ViewModels MUST NOT import material.dart or dart:ui', () {
      final violations = <String>[];
      final vmFiles = allDartFiles(uiDir).where((f) => f.path.contains('view_model'));

      for (final file in vmFiles) {
        final content = file.readAsStringSync();
        if (content.contains("import 'package:flutter/material.dart'") || 
            content.contains("import 'dart:ui'")) {
          violations.add('${file.path}: ViewModels must be pure logic. UI dependencies are forbidden.');
        }
      }
      if (violations.isNotEmpty) fail('Architectural Violations (MVVM): \n${violations.join('\n')}');
    });

    test('RULE: Models MUST use Equatable for value equality', () {
      final violations = <String>[];
      final modelFiles = allDartFiles(domainDir).where((f) => !f.path.contains('.g.dart'));
      ...
      if (violations.isNotEmpty) fail('Architectural Violations (Equality): \n${violations.join('\n')}');
    });

    test('RULE: No raw JSON access in Repositories (Mandatory DTOs)', () {
      final violations = <String>[];
      final repoFiles = allDartFiles(dataDir).where((f) => f.path.contains('repository'));
      ...
      for (final file in repoFiles) {
        ...
        if (hasMapAccess) {
          violations.add('${file.path}: Repositories should not access raw JSON keys. Use DTOs and Mappers.');
        }
      }
      if (violations.isNotEmpty) fail('Architectural Violations (Data Layer): \n${violations.join('\n')}');
    });
    ...
    test('RULE: One StatelessWidget per file and No Private Build Methods', () {
      ...
      for (final file in allDartFiles(uiDir)) {
        ...
        final widgetMatches = RegExp(r'class\s+\w+\s+extends\s+StatelessWidget').allMatches(content);
        if (widgetMatches.length > 1) {
          violations.add('${file.path}: Contains ${widgetMatches.length} StatelessWidgets. Use one per file.');
        }

        final buildMatches = RegExp(r'Widget\s+_\w+\s*\(').allMatches(content);
        if (buildMatches.isNotEmpty) {
          violations.add('${file.path}: Contains private build methods (_build...). Extract to separate classes.');
        }
      }
      if (violations.isNotEmpty) fail('Architectural Violations (Atomic UI): \n${violations.join('\n')}');
    });
      ...
      for (final dir in folders) {
        ...
        for (final sub in subDirs) {
          ...
          if (forbiddenNames.contains(folderName)) {
            fail('Folder name violation: ${sub.path}. Use plural names (e.g., view_models, mappers, models).');
          }
        }
      }
    });
