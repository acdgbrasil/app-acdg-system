...
class FakeGetPatientUseCase extends GetPatientUseCase {
  FakeGetPatientUseCase()
      : super(patientRepository: FakePatientRepository());
...
void main() {
  ...
  group('FamilyCompositionViewModel Gold Standard Refactor', () {
    ...
    test('Refactor: ageProfile calculation must be moved to a Domain Service or Entity', () {
    ...
    test('Refactor: No raw JSON parsing in ViewModel', () async {
      ...
      expect(viewModel.members.first.relationshipLabel, isNot(equals('json_error')));
    });
...
Patient _createRichPatient() {
  ...
  switch (PatientId.create('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa')) {
    ...
    case Failure(:final error): throw StateError('Test setup failed: $error');
  }
  ...
  switch (LookupId.create('33333333-3333-3333-3333-333333333333')) {
    case Success(:final value): prRelId = value;
    case Failure(:final error): throw StateError('Test setup failed: $error');
  }
