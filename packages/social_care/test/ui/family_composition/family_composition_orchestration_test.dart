  ...
  group('FamilyCompositionViewModel Orchestration (Mission 008.1)', () {
    ...
    setUp(() async {
      ...
      final pId = switch (PatientId.create(patientIdStr)) {
        ...
        Failure(:final error) => throw StateError('PatientId Setup failed: $error'),
      };

      final personId = switch (PersonId.create(personIdStr)) {
        Success(:final value) => value,
        Failure(:final error) => throw StateError('PersonId Setup failed: $error'),
      };

      final prRelId = switch (LookupId.create('dddddddd-dddd-dddd-dddd-dddddddddddd')) {
        Success(:final value) => value,
        Failure(:final error) => throw StateError('LookupId Setup failed: $error'),
      };
    ...
    test('handleModalSave (New Member): should execute add command and reload', () async {
      ...
      expect(viewModel.addMemberCommand.completed, isTrue, reason: 'Add command failed. Result: ${viewModel.addMemberCommand.result}');
      expect(viewModel.loadPatientCommand.completed, isTrue, reason: 'Reload failed after add');
    });
    ...
    test('handleModalSave (Edit Member): should remove old and add new', () async {
      ...
      expect(viewModel.removeMemberCommand.completed, isTrue, reason: 'Remove old member failed');
      expect(viewModel.addMemberCommand.completed, isTrue, reason: 'Add replacement failed');
    });

    test('handleRemove: should guard reference person and not execute command', () async {
      final pr = _createMockMember(id: '22222222-2222-2222-2222-222222222222', isPr: true);
      
      final removed = await viewModel.handleRemove(pr);
      
      expect(removed, isFalse, reason: 'Should not allow removing reference person');
      expect(viewModel.removeMemberCommand.result, isNull, reason: 'Command should not have run');
    });
  });
}
