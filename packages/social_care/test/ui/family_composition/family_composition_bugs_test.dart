...
void main() {
  ...
  group('ACDG Gold Standard: Contract & Performance Regression', () {
    test(
      'BUG 1 & 9: canSave MUST be false after load and true only after change',
      () async {
        ...
        expect(
          viewModel.canSave,
          isFalse,
          reason: 'canSave deve ser false se o estado for igual ao do banco',
        );
    ...
    test(
      'BUG 4 & 7: familyMemberToJson MUST strictly use key "relationship" (Contract Alignment)',
      () {
        final memberId = switch (PersonId.create(
        ...
        final member = FamilyMember.reconstitute(
          personId: memberId,
          relationshipId: LookupId.create(
            '00000000-0000-0000-0000-000000000001',
          ).valueOrNull!,
        ...
        expect(
          json.containsKey('relationship'),
          isTrue,
          reason: 'O servidor exige "relationship" no JSON',
        );
        expect(
          json.containsKey('relationshipId'),
          isFalse,
          reason: 'Não envie "relationshipId", evite poluição de contrato',
        );
      },
    );

    test(
      'PERFORMANCE: ageProfile MUST be memoized (computed only on data change)',
      () async {
        await viewModel.loadPatientCommand.execute();
        ...
        expect(
          identical(profile1, profile2),
          isTrue,
          reason:
              'ageProfile deve ser cacheado para evitar loops excessivos no build',
        );
      },
    );

    test(
      'BUG 17: familyMemberFromJson MUST support both "relationship" and "relationshipId"',
      () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'Deve suportar a nova chave "relationship" vinda do Sync Queue',
        );
        final member = (result as Success<FamilyMember>).value;
        expect(
          member.relationshipId.value,
