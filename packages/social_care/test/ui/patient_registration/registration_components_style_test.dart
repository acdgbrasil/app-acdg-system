...
void main() {
  group('Patient Registration Components Style & Architecture TDD', () {
    ...
    test(
      'FamilyMemberModal should not contain ScaffoldMessenger (View Logic Leak)',
      () {
        final file = File(
          p.join(
            componentsDir.path,
        ...
        expect(
          ...
          isFalse,
          reason:
              'ScaffoldMessenger found in FamilyMemberModal. View should not handle business validation messages directly.',
        );
      },
    );
    ...
    test('FamilyMemberModal should not manage FamilyMemberEntry locally', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            'View is managing its own FormState. Inject it or move to ViewModel.',
      );
    });
    ...
    test('RegistrationErrorModal should not contain hardcoded strings', () {
      ...
      final content = file.readAsStringSync();
      expect(
        content.contains("'Sem conexão'"),
        isFalse,
        reason: 'Hardcoded string found.',
      );
      expect(
        content.contains("'Erro no servidor'"),
        isFalse,
        reason: 'Hardcoded string found.',
      );
    });
    ...
    test(
      'Registration components should not build custom radio/checkbox without Design System',
      () {
        final file = File(
          p.join(
            componentsDir.path,
        ...
        expect(
          ...
          isFalse,
          reason:
              'Manual Radio Button detected. Use AcdgRadioButton from design_system.',
        );
      },
    );
  });
