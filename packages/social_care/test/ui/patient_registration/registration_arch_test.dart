...
void main() {
  group('Patient Registration Arch TDD (Mission 009.1)', () {
    ...
    test('FamilyMemberModal should NOT contain View Logic Leaks (ScaffoldMessenger, Navigator)', () {
      final file = File(p.join(componentsDir.path, 'forms', 'reference_person', 'family_member_modal.dart'));
      if (!file.existsSync()) return;
      
      final content = file.readAsStringSync();
      expect(content.contains('ScaffoldMessenger.of'), isFalse, 
        reason: 'CRITICAL FAILURE: ScaffoldMessenger found in FamilyMemberModal. The View should not handle business validation messages directly.');
    });
    ...
    test('FamilyMemberModal should NOT manage Local State directly', () {
      ...
      expect(content.contains('_entry = FamilyMemberEntry()'), isFalse, 
        reason: 'CRITICAL FAILURE: View is managing its own FormState. Inject it or move to the parent ViewModel.');
    });
    ...
    test('RegistrationErrorModal should NOT contain Hardcoded Strings', () {
      ...
      expect(content.contains(RegExp(r"'Sem conexão'|'Erro no servidor'")), isFalse, 
        reason: 'CRITICAL FAILURE: Hardcoded Portuguese strings found in the UI. Move to constants/l10n file.');
    });
    ...
    test('Registration components should NOT use Hardcoded Colors', () {
      ...
      for (final file in files) {
        ...
        expect(
          ...
          isFalse, 
          reason: 'CRITICAL FAILURE: Hardcoded Color found in ${file.path}. Use AppColors tokens exclusively.'
        );
      }
    });
    ...
    test('Registration components MUST use Design System for Inputs', () {
      ...
      expect(content.contains('BoxShape.circle'), isFalse, 
        reason: 'CRITICAL FAILURE: Manual Radio Button drawing detected. Use AcdgRadioButton from the design_system.');
    });
  });
}
