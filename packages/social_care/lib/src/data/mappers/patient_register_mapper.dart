...
import '../commands/register_patient_intent.dart';

/// Maps a [RegisterPatientIntent] into a fully assembled [Patient] domain
/// aggregate. Dedicated mapper for the Register Patient endpoint.
abstract final class PatientRegisterMapper {
  ...
  static Result<Patient> toPatient(RegisterPatientIntent intent) {
    ...
    if (intent.cns != null && intent.cns!.isNotEmpty) {
      final res = Cns.create(number: intent.cns!);
    ...
    switch (LookupId.create(intent.prRelationshipId)) {
      case Success(:final value):
        prRelId = value;
      case Failure(:final error):
        return Failure(error);
    ...
    switch (PatientId.create(UuidUtil.generateV4())) {
      case Success(:final value):
        patientId = value;
      case Failure(:final error):
        return Failure(error);
    ...
    switch (PersonId.create(personIdStr)) {
      case Success(:final value):
        personId = value;
      case Failure(:final error):
        return Failure(error);
    ...
    final FamilyMember prMember;
    switch (FamilyMember.create(
      personId: personId,
