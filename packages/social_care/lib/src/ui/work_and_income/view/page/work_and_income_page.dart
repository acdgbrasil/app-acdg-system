...
class WorkAndIncomePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<WorkAndIncomePage> createState() => _WorkAndIncomePageState();
}

class _WorkAndIncomePageState extends ConsumerState<WorkAndIncomePage> {
  @override
  void initState() { super.initState(); ref.read(workAndIncomeViewModelProvider(widget.patientId)).loadCommand.execute(); }

  @override
  Widget build(BuildContext context) {
    final vm = ref.watch(workAndIncomeViewModelProvider(widget.patientId));
    return Scaffold(body: SafeArea(child: Column(children: [
      Padding(padding: const EdgeInsets.symmetric(horizontal: 48, vertical: 16), child: Row(children: [
        ...
        const SizedBox(width: 24), Text(WorkAndIncomeL10n.navFamilies, style: TextStyle(fontFamily: 'Satoshi', fontWeight: FontWeight.w700, fontSize: 14, color: AppColors.textPrimary.withValues(alpha: 0.55))),
        const SizedBox(width: 24), const Text(WorkAndIncomeL10n.navRegistration, style: TextStyle(fontFamily: 'Satoshi', fontWeight: FontWeight.w700, fontSize: 14, color: AppColors.textPrimary, decoration: TextDecoration.underline, decorationColor: AppColors.textPrimary)),
      ])),
      ListenableBuilder(listenable: vm, builder: (context, _) => Padding(padding: const EdgeInsets.fromLTRB(48, 4, 48, 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text(WorkAndIncomeL10n.pageTitle, style: TextStyle(fontFamily: 'Satoshi', fontWeight: FontWeight.w700, fontSize: 38, letterSpacing: -1, color: AppColors.textPrimary)),
      ...
      Expanded(child: ListenableBuilder(listenable: vm.loadCommand, builder: (context, _) {
        ...
        return WorkAndIncomeContent(viewModel: vm);
      })),
      ListenableBuilder(listenable: vm, builder: (context, _) => Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          TextButton(onPressed: () { if (context.canPop()) context.pop(); else context.go('/social-care'); }, style: TextButton.styleFrom(foregroundColor: AppColors.danger, shape: const StadiumBorder(), padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.close, size: 16), SizedBox(width: 7), Text(WorkAndIncomeL10n.btnCancel, style: TextStyle(fontFamily: 'Playfair Display', fontStyle: FontStyle.italic, fontSize: 14))])),
          FilledButton(onPressed: vm.canSave ? () => vm.saveCommand.execute() : null, style: FilledButton.styleFrom(backgroundColor: AppColors.primary, disabledBackgroundColor: AppColors.primary.withValues(alpha: 0.4), shape: const StadiumBorder(), padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [Text(WorkAndIncomeL10n.btnSave, style: TextStyle(fontFamily: 'Playfair Display', fontStyle: FontStyle.italic, fontSize: 14, color: AppColors.background)), SizedBox(width: 7), Icon(Icons.check, size: 16, color: AppColors.background)])),
        ]),
      )),
    ])));
  }
