...
class ViolationReportViewModel extends BaseViewModel {
  ...
  String? _victimId;
  String? get victimId => _victimId;
  String? _violationType;
  String? get violationType => _violationType;
  String _descriptionOfFact = '';
  String get descriptionOfFact => _descriptionOfFact;
  ...
  bool _validateDates() {
    ...
    if (report.isAfter(DateTime.now().add(const Duration(days: 1)))) {
      return false;
    }
  ...
  void updateReportDate(String v) {
    _reportDate = v;
    notifyListeners();
  }

  void updateIncidentDate(String v) {
    _incidentDate = v;
    notifyListeners();
  }

  void updateVictim(String v) {
    _victimId = v;
    notifyListeners();
  }

  void updateViolationType(String v) {
    _violationType = v;
    notifyListeners();
  }

  void updateDescription(String v) {
    _descriptionOfFact = v;
    notifyListeners();
  }

  void updateActions(String v) {
    _actionsTaken = v;
    notifyListeners();
  }
  ...
  Future<Result<void>> _save() async {
    ...
    DateTime? incDate;
    if (_incidentDate.isNotEmpty) {
      incDate = DateTime.tryParse(_incidentDate);
    }

    final ViolationType vt;
    try {
      vt = ViolationType.values.byName(_violationType!);
    } on ArgumentError {
      return Failure(
        AppError(
          code: 'VR-001',
          message: 'Tipo de violação inválido: $_violationType',
          module: 'social-care/violation-report',
    ...
    final intent = ReportViolationIntent(
      ...
      victimId: _victimId!,
      violationType: vt,
      descriptionOfFact: _descriptionOfFact.trim(),
      incidentDate: incDate,
      actionsTaken: _actionsTaken.trim().isEmpty ? null : _actionsTaken.trim(),
    );
