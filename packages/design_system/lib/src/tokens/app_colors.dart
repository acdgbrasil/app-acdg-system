import 'package:flutter/painting.dart';

/// Design tokens for the application's color palette.
///
/// Aligned with the ACDG brand identity: navy blue, purple accent,
/// cool whites — matching the splash screen and logo.
abstract final class AppColors {
  // Main Backgrounds
  static const Color background = Color(0xFFF0F0FA); // Cool white/lavender
  static const Color backgroundDark = Color(
    0xFF1B2A6B,
  ); // Deep navy (popup/modal)
  static const Color cardAlternate = Color(
    0xFFE0E2F0,
  ); // Light blue-gray (card alternation)

  // Typography & Borders
  static const Color textPrimary = Color(0xFF1B2A6B); // Deep navy
  static const Color textOnDark = Color(
    0xFFF0F0FA,
  ); // Cool white (text on dark backgrounds)
  static const Color textMuted = Color(
    0x801B2A6B,
  ); // Navy with 50% opacity
  static const Color textAntiFlash = Color(
    0xFFEBEBEB,
  ); // Anti Flash (labels inside desktop popup)
  static const Color textBlack = Color(
    0xFF000000,
  ); // Absolute black (documents needed labels)

  // Semantic Colors
  static const Color primary = Color(0xFF3A5FC8); // Blue (main action)
  static const Color accent = Color(0xFF7B5EA7); // Purple (ACDG brand accent)
  static const Color danger = Color(0xFFA6290D); // Red (Cancel/Clear)
  static const Color warning = Color(0xFFC9960A); // Gold/Amber (caregiver star)
  static const Color surface = Color(0xFFF8F9FF); // Lightest cool white (FAB, dialogs)
  static const Color surfaceLight = Color(
    0xFFFCFCFF,
  ); // Almost white with blue tint (icons on dark, toast text)

  // Overlays
  static const Color barrierDark = Color(0x591B2A6B); // Navy barrier overlay

  // Interactions & Elements
  static const Color border = Color(0xFF1B2A6B); // Deep navy
  static const Color borderOnDark = Color(0xFFF0F0FA); // Cool white
  static const Color inputLine = Color(
    0x331B2A6B,
  ); // Navy with ~20% opacity (underlines)
  static const Color shadow = Color(
    0x0D121217,
  ); // rgba(18,18,23,0.05) - used in checkboxes
  static const Color buttonShadow = Color(
    0x1F000000,
  ); // rgba(0,0,0,0.12) - button shadow

  // Elevation & Glow Shadows
  static const Color shadowSubtle = Color(
    0x0F000000,
  ); // rgba(0,0,0,0.06) - secondary button shadow layer
  static const Color popupGlow = Color(
    0x33F0F0FA,
  ); // rgba(240,240,250,0.2) - popup save button glow
  static const Color elevationXs = Color(
    0x0A000000,
  ); // rgba(0,0,0,0.04) - popup shadow layers 1-2
  static const Color elevationSm = Color(
    0x14000000,
  ); // rgba(0,0,0,0.08) - popup shadow layer 3
  static const Color elevationMd = Color(
    0x29000000,
  ); // rgba(0,0,0,0.16) - popup shadow layer 4
  static const Color elevationLg = Color(
    0x3D000000,
  ); // rgba(0,0,0,0.24) - popup shadow layer 5
  static const Color elevationXl = Color(
    0x7A000000,
  ); // rgba(0,0,0,0.48) - popup shadow layer 6
}
